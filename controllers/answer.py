...
def quickanswer():
    ...
    messagetxt = f'Answer recorded for item: {questid} '
...
def perccomplete():
    ...
    quest.update_record()
    db.commit()
    return f'Action updated to {percentcomplete}'
...
def like(itemid, table='question'):
    ...
    if table == 'question':
        ...
        numlikes = f'<div id="btns{itemid}" hx-swap-oob="true"> <sub>Likes:{likecount}</sub></div>'
    db.commit()
